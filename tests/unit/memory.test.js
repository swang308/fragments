...
describe('MemoryDB unit tests', () => {
  // Test writeFragment
  test('1. writeFragment should write fragment metadata to memory', async () => {
    const result = await writeFragment(mockFragment);
    expect(result).toBeUndefined(); // As write operations usually return nothing
  });

  // Test readFragment
  test('2. readFragment should return the correct fragment metadata from memory', async () => {
  ...
  test('3. writeFragmentData should write fragment data to memory', async () => {
    const result = await writeFragmentData(mockFragment.ownerId, mockFragment.id, mockDataBuffer);
    expect(result).toBeUndefined(); // As write operations usually return nothing
  });

  // Test readFragmentData
  test('4. readFragmentData should return the correct fragment data buffer from memory', async () => {
  ...
  test('5. readFragment should return undefined if fragment does not exist', async () => {
    const result = await readFragment('user1', 'nonexistent-fragment');
    expect(result).toBeUndefined();
  });

  test('6. readFragmentData should return undefined if data does not exist', async () => {
