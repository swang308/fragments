...
describe('POST /v1/fragments', () => {
  ...
  // text/plain
  test('3. Authenticated users can post text/plain fragment data', async () => {
    const data = Buffer.from('hello');
  ...
  // application/json
  test('4. uthenticated users can post application/json fragment data', async () => {
    const data = Buffer.from('hello');
    const res = await request(app)
      .post('/v1/fragments')
      .auth('user1@email.com', 'password1')
  ...
  test('5. Return 415 for unsupported content types', async () => {
  ...
  test('6. Return 401 for missing required fields', async () => {
  ...
  test('7. Return 401 for invalid JSON data format', async () => {
